...
{
    chunk_t *h_next, *new_val_h_next, *ch_next;
    ch_next    = ch->next;
    new_val_h_next = head->next;
    do { ch->next = h_next = new_val_h_next; WMB_NEAR_CAS(); }
    while ( (new_val_h_next = CASPO(&head->next, h_next, ch_next)) != h_next );
}
...
{
    int i;
    chunk_t *new_val_rh, *rh, *rt, *head;

 retry:
    head = gc_global.free_chunks;
    new_val_rh = head->next;
    do {
        rh = new_val_rh;
        rt = head;
        WEAK_DEP_ORDER_RMB();
    ...
    while ( (new_val_rh = CASPO(&head->next, rh, rt->next)) != rh );
 ...
 *   mprotect(async_page, <page size>, <new_val flags>)
...
{
    chunk_t *alloc, *p, *new_val_p, *nh;
    unsigned int sz;

    alloc = gc_global.alloc[i];
    new_val_p = alloc->next;
    ...
    while ( (new_val_p = CASPO(&alloc->next, p, p->next)) != p );
...
void gc_free(ptst_t *ptst, void *p, int alloc_id) 
{
#ifndef MINIMAL_GC
    gc_t *gc = ptst->gc;
    chunk_t *prev, *new_val, *ch = gc->garbage[gc->epoch][alloc_id];
    ...
    else if ( ch->i == BLKS_PER_CHUNK )
    {
        ...
        new_val  = chunk_from_cache(gc);
        gc->garbage[gc->epoch][alloc_id] = new_val;
        new_val->next  = ch;
        prev->next = new_val;
        ch = new_val;
    }
...
void gc_enter(ptst_t *ptst)
{
 ...
 retry:
    ...
    if ( cnt == 1 )
    {
        new_val_epoch = gc_global.current;
        if ( gc->epoch != new_val_epoch )
        {
            gc->epoch = new_val_epoch;
