...
setval_t set_update(set_t *s, unsigned long k, setval_t v, int overwrite)
{
    setval_t ov, nov;
    node_t *p, *n, *new_val = NULL, **ppc;
    ptst_t *ptst;
    ...
    retry:
        ...
        if ( new_val == NULL )
        {
            new_val = (node_t *)(gc_alloc(ptst, gc_id));
            new_val->k = k;
            new_val->v = v;
        }
    ...
    while ( CASPO(ppc, n, new_val) != n );

    new_val = NULL;

 out:
    if ( new_val ) gc_free(ptst, new_val, gc_id);
    critical_exit(ptst);
    return ov;
}
