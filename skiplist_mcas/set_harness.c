...
volatile bool_t go = FALSE;
volatile int threads_initialised1 = 0;
static int max_key, log_max_key;
volatile int threads_initialised2 = 0;
volatile int threads_initialised3 = 0;
int num_threads;
...
static void *thread_start(void *arg)
{
    ...
    //unsigned long val_key = 0xdeaddead;
    //v = &val_key;
    //unsigned long *tmp = (unsigned long*)&v;
    //printf("\nThe value of v in set_harness is %lu\n", *tmp);  
#ifndef DO_WRITE_LOG
    ...
    for ( i = (_max_key / num_threads); i > 0; i >>= 1 )
    {
        for ( k = i >> 1; k < (_max_key / num_threads); k += i )
        {
            set_update(shared.set, 
                       /*(unsigned long)*/(k + id * (_max_key / num_threads)), 
                       (void *)(0xdeadbee0), 1);
        }
    }
#endif
    ...
    for ( i = 0; (i < MAX_ITERATIONS) && !shared.alarm_time; i++ )
    {
        ...
        log->start = my_int;
#endif
	//printf("\nThe value of r is %llu\n", r);
        if ( ((r>>4)&255) < prop )
        {
            //printf("\nGoing to the set lookup function\n");
            ov = v = set_lookup(shared.set, /*(unsigned long)*/(k));
        }
        else if ( ((r>>12)&1) )
        {
            //printf("\nGoing to the set update function\n");
            v = (void *)((r&~7)|0x8);
            ov = set_update(shared.set, /*(unsigned long)*/(k), /*(void*)*/(v), 1);
        }
        else
        {
            //printf("\nGoing to the remove function\n");
            v = NULL;
            ov = set_remove(shared.set, /*(unsigned long)*/(k));
        }
...
static void test_multithreaded (void)
{
 ...
 skip_thread_creation:
    ...
    log_float ((char *)("wall_time_s"), wall_time);
    log_float ((char *)("user_time_s"), user_time);
    log_float ((char *)("system_time_s"), sys_time);
    ...
    log_int ((char *)("min_successes"), min_successes);
    log_int ((char *)("max_successes"), max_successes);
    log_int ((char *)("num_successes"), num_successes);

    log_float((char *)("us_per_success"), (num_threads*wall_time/**1000000.0*/)/num_successes);

    log_int((char *)("log max key"), log_max_key);
}
...
int main (int argc, char **argv)
{
    ...
    log_int ((char *)("num_threads"), num_threads);

    proportion = atoi(argv[2]);
    log_float ((char *)("frac_reads"), (float)proportion/256.0);

    log_max_key = atoi(argv[3]);
    max_key = 1 << atoi(argv[3]);
    log_int((char *)("max_key"), max_key);

    log_int ((char *)("max_iterations"), MAX_ITERATIONS);

    log_int ((char *)("wall_time_limit_s"), MAX_WALL_TIME);
...
/*#if defined(INTEL)
    {
        ...
        act.sa_handler = (__sighandler_t)tstp_handler;
